...
func recordSubscribeDropped(device string) {
	RecordSubscribeDropped(device)
}

// RecordSubscribeDropped bumps the per-device drop counter. Exported so
// platform-specific transports that live outside this package can share the
// same metric without re-owning the metric variable.
func RecordSubscribeDropped(device string) {
