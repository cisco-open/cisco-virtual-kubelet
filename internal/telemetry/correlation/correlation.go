...
func NewCache(ttl time.Duration, cap int) *Cache {
	if ttl <= 0 {
		ttl = DefaultTTL
	}
	if cap <= 0 {
...
func (c *Cache) Upsert(device, appID string, sc trace.SpanContext) {
	...
	elem := c.order.PushFront(&entry{key: key, sc: sc, expiresAt: now.Add(c.ttl)})
	c.items[key] = elem
...
func (c *Cache) Get(device, appID string) (trace.SpanContext, bool) {
