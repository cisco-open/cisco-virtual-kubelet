...
- **Drivers** - `XE` is production-focused; `NXOS` supports NX-OS
  app-hosting through NX-API; `FTD` onboards Cisco Secure Firewall Threat
  Defense for health, operations, telemetry, and observability without
  app-hosting; `FAKE` is for testing; `XR` and `OPENCONFIG` remain reserved
  driver names in the API surface.
