# cdp

Status: **implemented** — CVK writes the managed leaves listed below on every reconcile.

- Shape: `singleton`
- netascode portal: [https://netascode.cisco.com/docs/data_models/iosxe/device/cdp/](https://netascode.cisco.com/docs/data_models/iosxe/device/cdp/)

## YANG paths

- `/Cisco-IOS-XE-native:native/cdp`


## Managed leaves

The writer reads and writes the following leaves. Leaves outside this
set present on the device are preserved (additive merge).

- `run`
- `advertise_v2`
