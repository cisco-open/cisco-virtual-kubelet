...
// FMCConfigSpec declares the desired FMC configuration for a single CiscoDevice.
// The source payload follows the Network as Code FMC model: either a full
// `fmc:` envelope or the inner FMC configuration block can be supplied. When a
// full model includes top-level `existing.fmc`, that read-only reference model
// is preserved under the canonical `existing` family for resolvers.
type FMCConfigSpec struct {
	// DeviceRef targets the CiscoDevice this configuration applies to.
	// +kubebuilder:validation:Required
	DeviceRef DeviceRef `json:"deviceRef"`

	// ManagedFamilies is the closed list of NetAsCode FMC top-level families
	// this CR owns. Supported families are nac_configuration, existing, system,
	// domains, devices, objects, policies, vpns, and integrations.
	// +kubebuilder:validation:Required
	// +kubebuilder:validation:MinItems=1
	// +listType=set
	ManagedFamilies []string `json:"managedFamilies"`

	// Source carries the per-controller Network as Code FMC configuration payload.
	// +kubebuilder:validation:Required
	...
	// resolved FMC intent from Kubernetes Secrets, so shared secrets and API
	// credentials never need to live in a ConfigMap or git-tracked YAML.
	// +optional
	SecretRefs []FamilySecretRef `json:"secretRefs,omitempty"`
}

// FMCConfigStatus reports reconciliation state to users and GitOps agents.
type FMCConfigStatus struct {
	...
	// LastDeviceCheck records the most recent reconcile tick that reached FMC.
	// +optional
...
// FMCConfig is the per-controller desired-configuration CR for Cisco FMC.
//
// +kubebuilder:object:root=true
// +kubebuilder:subresource:status
// +kubebuilder:resource:scope=Namespaced,shortName=fmccfg
...
type FMCConfig struct {
	...
	Spec   FMCConfigSpec   `json:"spec"`
	Status FMCConfigStatus `json:"status,omitempty"`
}

// FMCConfigList is the list type for FMCConfig.
//
// +kubebuilder:object:root=true
type FMCConfigList struct {
	metav1.TypeMeta `json:",inline"`
	metav1.ListMeta `json:"metadata,omitempty"`
	Items           []FMCConfig `json:"items"`
}

func init() {
	SchemeBuilder.Register(&FMCConfig{}, &FMCConfigList{})
}
